...
impl VideoPlayer {
    ...
    pub fn State(&self)->Result<MediaState>{let f:sys::cna_video_player_get_state_fn=unsafe{native_function(self.core.native().media.video_player_get_state)};let mut value=0;self.core.native().check(unsafe{f(self.core.handle()?,&mut value)})?;MediaState::from_native(value)}
    pub fn PlayPosition(&self)->Result<TimeSpan>{let f:sys::cna_video_player_get_play_position_ticks_fn=unsafe{native_function(self.core.native().media.video_player_get_play_position_ticks)};let mut value=0;self.core.native().check(unsafe{f(self.core.handle()?,&mut value)})?;Ok(TimeSpan::from_ticks(value))}
    pub fn IsLooped(&self)->Result<bool>{Ok(*self.is_looped.lock().unwrap_or_else(std::sync::PoisonError::into_inner))}
    pub fn SetIsLooped(&self,value:bool)->Result<()>{let f:sys::cna_video_player_set_is_looped_fn=unsafe{native_function(self.core.native().media.video_player_set_is_looped)};self.core.native().check(unsafe{f(self.core.handle()?,u8::from(value))})?;*self.is_looped.lock().unwrap_or_else(std::sync::PoisonError::into_inner)=value;Ok(())}
    pub fn IsMuted(&self)->Result<bool>{Ok(*self.is_muted.lock().unwrap_or_else(std::sync::PoisonError::into_inner))}
    pub fn SetIsMuted(&self,value:bool)->Result<()>{let f:sys::cna_video_player_set_is_muted_fn=unsafe{native_function(self.core.native().media.video_player_set_is_muted)};self.core.native().check(unsafe{f(self.core.handle()?,u8::from(value))})?;*self.is_muted.lock().unwrap_or_else(std::sync::PoisonError::into_inner)=value;Ok(())}
    pub fn Volume(&self)->Result<f32>{Ok(*self.volume.lock().unwrap_or_else(std::sync::PoisonError::into_inner))}
    pub fn SetVolume(&self,value:f32)->Result<()>{if value<0.0||value>1.0{return Err(CnaError::InvalidInput("VideoPlayer Volume must be in [0, 1] or NaN"));}let f:sys::cna_video_player_set_volume_fn=unsafe{native_function(self.core.native().media.video_player_set_volume)};self.core.native().check(unsafe{f(self.core.handle()?,value)})?;*self.volume.lock().unwrap_or_else(std::sync::PoisonError::into_inner)=value;Ok(())}
    pub fn Play(&self,video:Arc<Video>)->Result<()>{if video.core.generation()!=self.core.generation(){return Err(CnaError::InvalidInput("Video belongs to another Game generation"));}let f:sys::cna_video_player_play_fn=unsafe{native_function(self.core.native().media.video_player_play)};self.core.native().check(unsafe{f(self.core.handle()?,video.native_handle()?)})?;*self.video.lock().unwrap_or_else(std::sync::PoisonError::into_inner)=Some(video);Ok(())}
    ...
    fn operation(&self,slot:usize)->Result<()>{let f:unsafe extern "C" fn(sys::CNA_VideoPlayerHandle)->sys::CNA_Result=unsafe{native_function(slot)};self.core.native().check(unsafe{f(self.core.handle()?)})}
    pub fn GetTexture(&self)->Result<Option<Texture2D>>{if self.video.lock().unwrap_or_else(std::sync::PoisonError::into_inner).is_none(){return Err(CnaError::InvalidInput("VideoPlayer has no current Video"));}let f:sys::cna_video_player_get_texture_fn=unsafe{native_function(self.core.native().media.video_player_get_texture)};let mut texture=0;let mut available=0;self.core.native().check(unsafe{f(self.core.handle()?,&mut texture,&mut available)})?;if available==0{Ok(None)}else{Err(CnaError::UnsupportedRuntime("CNA ABI 0.7 exposes a player-owned frame handle only until the next player call, but the selected Rust Texture2D model requires stable resource identity; the borrowed handle was not wrapped or destroyed"))}}
    pub fn Dispose(&self)->Result<()>{self.core.Dispose()}
    pub fn Finalize(&self)->Result<()> { Ok(()) }
}

impl Drop for VideoPlayer { fn drop(&mut self){self.core.invalidate();} }
