...
use crate::extensions::media::SongCollectionExt;
use crate::extensions::media::SongExt;

use super::collections::CollectionElement;
use super::runtime::MediaRuntime;
use super::{
    read_blob, read_bool, read_i32, read_i64, read_string, string_view,
     ...
     $equals_alias:ty, $equals_field:ident, $hash_alias:ty, $hash_field:ident) => {
        impl $type {
            pub fn Name(&self) -> Result<String> {
                let size: $name_alias = self.core.native().media.$name_field;
                let copy: $copy_alias = self.core.native().media.$copy_field;
            ...
            pub fn GetHashCode(&self) -> Result<i32> {
                let getter: $hash_alias = self.core.native().media.$hash_field;
                read_i32(&self.core, getter)
            }
            pub fn EqualsWithOther(&self, other: &$type) -> Result<bool> {
                let equals: $equals_alias = self.core.native().media.$equals_field;
...
impl Album {
    pub(crate) fn from_handle(native: Arc<Native>, runtime: Arc<MediaRuntime>, generation: u64, handle: sys::CNA_Handle) -> Arc<Self> {
        let dispose: sys::cna_album_dispose_fn = native.media.album_dispose;
        let destroy: sys::cna_album_destroy_fn = native.media.album_destroy;
        let is_disposed: sys::cna_album_get_is_disposed_fn = native.media.album_get_is_disposed;
    ...
    pub fn Artist(&self) -> Result<Option<Arc<Artist>>> { let f: sys::cna_album_get_artist_fn = self.core.native().media.album_get_artist; optional_child(&self.core, &self.artist, f, Artist::from_handle) }
    pub fn Genre(&self) -> Result<Option<Arc<Genre>>> { let f: sys::cna_album_get_genre_fn = self.core.native().media.album_get_genre; optional_child(&self.core, &self.genre, f, Genre::from_handle) }
    pub fn Songs(&self) -> Result<Arc<SongCollection>> { let f: sys::cna_album_get_songs_fn = self.core.native().media.album_get_songs; required_child(&self.core, &self.songs, f, SongCollection::from_handle) }
    pub fn Duration(&self) -> Result<TimeSpan> { let f: sys::cna_album_get_duration_fn = self.core.native().media.album_get_duration; Ok(TimeSpan::from_ticks(read_i64(&self.core, f)?)) }
    pub fn HasArt(&self) -> Result<bool> { let f: sys::cna_album_get_has_art_fn = self.core.native().media.album_get_has_art; read_bool(&self.core, f) }
    pub fn GetAlbumArt(&self) -> Result<Box<dyn Read + Send>> { let s: sys::cna_album_get_art_size_fn = self.core.native().media.album_get_art_size; let c: sys::cna_album_copy_art_fn = self.core.native().media.album_copy_art; read_blob(&self.core, s, c) }
    pub fn GetThumbnail(&self) -> Result<Box<dyn Read + Send>> { let s: sys::cna_album_get_thumbnail_size_fn = self.core.native().media.album_get_thumbnail_size; let c: sys::cna_album_copy_thumbnail_fn = self.core.native().media.album_copy_thumbnail; read_blob(&self.core, s, c) }
...
impl Artist {
    pub(crate) fn from_handle(native: Arc<Native>, runtime: Arc<MediaRuntime>, generation: u64, handle: sys::CNA_Handle) -> Arc<Self> { let d:sys::cna_artist_dispose_fn=native.media.artist_dispose; let x:sys::cna_artist_destroy_fn=native.media.artist_destroy; let i:sys::cna_artist_get_is_disposed_fn=native.media.artist_get_is_disposed; Arc::new(Self{core:ResourceCore::new(native,runtime,generation,handle,Some(d),x,Some(i)),albums:Mutex::new(None),songs:Mutex::new(None)}) }
    pub fn Albums(&self)->Result<Arc<AlbumCollection>>{let f:sys::cna_artist_get_albums_fn=self.core.native().media.artist_get_albums;required_child(&self.core,&self.albums,f,AlbumCollection::from_handle)}
    pub fn Songs(&self)->Result<Arc<SongCollection>>{let f:sys::cna_artist_get_songs_fn=self.core.native().media.artist_get_songs;required_child(&self.core,&self.songs,f,SongCollection::from_handle)}
...
impl Genre {
    pub(crate) fn from_handle(native: Arc<Native>, runtime: Arc<MediaRuntime>, generation: u64, handle: sys::CNA_Handle) -> Arc<Self> { let d:sys::cna_genre_dispose_fn=native.media.genre_dispose;let x:sys::cna_genre_destroy_fn=native.media.genre_destroy;let i:sys::cna_genre_get_is_disposed_fn=native.media.genre_get_is_disposed;Arc::new(Self{core:ResourceCore::new(native,runtime,generation,handle,Some(d),x,Some(i)),albums:Mutex::new(None),songs:Mutex::new(None)}) }
    pub fn Albums(&self)->Result<Arc<AlbumCollection>>{let f:sys::cna_genre_get_albums_fn=self.core.native().media.genre_get_albums;required_child(&self.core,&self.albums,f,AlbumCollection::from_handle)}
    pub fn Songs(&self)->Result<Arc<SongCollection>>{let f:sys::cna_genre_get_songs_fn=self.core.native().media.genre_get_songs;required_child(&self.core,&self.songs,f,SongCollection::from_handle)}
    fn invalidate_children(&self){if let Some(v)=self.albums.lock().unwrap_or_else(std::sync::PoisonError::into_inner).take(){v.invalidate();}if let Some(v)=self.songs.lock().unwrap_or_else(std::sync::PoisonError::into_inner).take(){v.invalidate();}}
}
...
impl Playlist {
    pub(crate) fn from_handle(native: Arc<Native>, runtime: Arc<MediaRuntime>, generation: u64, handle: sys::CNA_Handle)->Arc<Self>{let d:sys::cna_playlist_dispose_fn=native.media.playlist_dispose;let x:sys::cna_playlist_destroy_fn=native.media.playlist_destroy;let i:sys::cna_playlist_get_is_disposed_fn=native.media.playlist_get_is_disposed;Arc::new(Self{core:ResourceCore::new(native,runtime,generation,handle,Some(d),x,Some(i)),songs:Mutex::new(None)})}
    pub fn Songs(&self)->Result<Arc<SongCollection>>{let f:sys::cna_playlist_get_songs_fn=self.core.native().media.playlist_get_songs;required_child(&self.core,&self.songs,f,SongCollection::from_handle)}
    pub fn Duration(&self)->Result<TimeSpan>{let f:sys::cna_playlist_get_duration_fn=self.core.native().media.playlist_get_duration;Ok(TimeSpan::from_ticks(read_i64(&self.core,f)?))}
...
impl Song {
    pub(crate) fn from_handle(native: Arc<Native>,runtime:Arc<MediaRuntime>,generation:u64,handle:sys::CNA_Handle)->Arc<Self>{let d:sys::cna_song_dispose_fn=native.media.song_dispose;let x:sys::cna_song_destroy_fn=native.media.song_destroy;let i:sys::cna_song_get_is_disposed_fn=native.media.song_get_is_disposed;Arc::new(Self{core:ResourceCore::new(native,runtime,generation,handle,Some(d),x,Some(i)),album:Mutex::new(None),artist:Mutex::new(None),genre:Mutex::new(None)})}
    pub fn FromUri(game:&GameContext<'_>,name:&str,uri:&str)->Result<Self>{let (native,game_handle)=game.native_game();let runtime=Arc::clone(game.media_runtime());let generation=game.media_generation();let f:sys::cna_song_create_from_uri_fn=native.media.song_create_from_uri;let mut handle=0;native.check(unsafe{f(game_handle,string_view(name),string_view(uri),&mut handle)})?;Arc::try_unwrap(Self::from_handle(Arc::clone(native),runtime,generation,handle)).map_err(|_|CnaError::InvalidInput("new Song identity was unexpectedly shared"))}
    pub fn Album(&self)->Result<Option<Arc<Album>>>{let f:sys::cna_song_get_album_fn=self.core.native().media.song_get_album;optional_child(&self.core,&self.album,f,Album::from_handle)}
    pub fn Artist(&self)->Result<Option<Arc<Artist>>>{let f:sys::cna_song_get_artist_fn=self.core.native().media.song_get_artist;optional_child(&self.core,&self.artist,f,Artist::from_handle)}
    pub fn Genre(&self)->Result<Option<Arc<Genre>>>{let f:sys::cna_song_get_genre_fn=self.core.native().media.song_get_genre;optional_child(&self.core,&self.genre,f,Genre::from_handle)}
    pub fn Duration(&self)->Result<TimeSpan>{let f:sys::cna_song_get_duration_fn=self.core.native().media.song_get_duration;Ok(TimeSpan::from_ticks(read_i64(&self.core,f)?))}
    pub fn IsProtected(&self)->Result<bool>{let f:sys::cna_song_get_is_protected_fn=self.core.native().media.song_get_is_protected;read_bool(&self.core,f)}
    pub fn IsRated(&self)->Result<bool>{let f:sys::cna_song_get_is_rated_fn=self.core.native().media.song_get_is_rated;read_bool(&self.core,f)}
    pub fn PlayCount(&self)->Result<i32>{let f:sys::cna_song_get_play_count_fn=self.core.native().media.song_get_play_count;read_i32(&self.core,f)}
    pub fn Rating(&self)->Result<i32>{let f:sys::cna_song_get_rating_fn=self.core.native().media.song_get_rating;read_i32(&self.core,f)}
    pub fn TrackNumber(&self)->Result<i32>{let f:sys::cna_song_get_track_number_fn=self.core.native().media.song_get_track_number;read_i32(&self.core,f)}
...
impl Picture {
    pub(crate) fn from_handle(native:Arc<Native>,runtime:Arc<MediaRuntime>,generation:u64,handle:sys::CNA_Handle)->Arc<Self>{let d:sys::cna_picture_dispose_fn=native.media.picture_dispose;let x:sys::cna_picture_destroy_fn=native.media.picture_destroy;let i:sys::cna_picture_get_is_disposed_fn=native.media.picture_get_is_disposed;Arc::new(Self{core:ResourceCore::new(native,runtime,generation,handle,Some(d),x,Some(i)),album:Mutex::new(None)})}
    pub fn Album(&self)->Result<Option<Arc<PictureAlbum>>>{let f:sys::cna_picture_get_album_fn=self.core.native().media.picture_get_album;optional_child(&self.core,&self.album,f,PictureAlbum::from_handle)}
    pub fn Date(&self)->Result<SystemTime>{let f:sys::cna_picture_get_date_unix_ticks_fn=self.core.native().media.picture_get_date_unix_ticks;let ticks=read_i64(&self.core,f)?;let nanos=ticks.unsigned_abs().checked_mul(100).ok_or(CnaError::InvalidInput("picture date is out of range"))?;let duration=Duration::from_nanos(nanos);if ticks>=0{UNIX_EPOCH.checked_add(duration)}else{UNIX_EPOCH.checked_sub(duration)}.ok_or(CnaError::InvalidInput("picture date is out of range"))}
    pub fn Width(&self)->Result<i32>{let f:sys::cna_picture_get_width_fn=self.core.native().media.picture_get_width;read_i32(&self.core,f)}
    pub fn Height(&self)->Result<i32>{let f:sys::cna_picture_get_height_fn=self.core.native().media.picture_get_height;read_i32(&self.core,f)}
    pub fn GetImage(&self)->Result<Box<dyn Read+Send>>{let s:sys::cna_picture_get_image_size_fn=self.core.native().media.picture_get_image_size;let c:sys::cna_picture_copy_image_fn=self.core.native().media.picture_copy_image;read_blob(&self.core,s,c)}
    pub fn GetThumbnail(&self)->Result<Box<dyn Read+Send>>{let s:sys::cna_picture_get_thumbnail_size_fn=self.core.native().media.picture_get_thumbnail_size;let c:sys::cna_picture_copy_thumbnail_fn=self.core.native().media.picture_copy_thumbnail;read_blob(&self.core,s,c)}
...
impl PictureAlbum {
    pub(crate) fn from_handle(native:Arc<Native>,runtime:Arc<MediaRuntime>,generation:u64,handle:sys::CNA_Handle)->Arc<Self>{let d:sys::cna_picture_album_dispose_fn=native.media.picture_album_dispose;let x:sys::cna_picture_album_destroy_fn=native.media.picture_album_destroy;let i:sys::cna_picture_album_get_is_disposed_fn=native.media.picture_album_get_is_disposed;Arc::new(Self{core:ResourceCore::new(native,runtime,generation,handle,Some(d),x,Some(i)),parent:Mutex::new(None),albums:Mutex::new(None),pictures:Mutex::new(None)})}
    pub fn Parent(&self)->Result<Option<Arc<super::PictureAlbum>>>{let f:sys::cna_picture_album_get_parent_fn=self.core.native().media.picture_album_get_parent;optional_child(&self.core,&self.parent,f,PictureAlbum::from_handle)}
    pub fn Albums(&self)->Result<Arc<PictureAlbumCollection>>{let f:sys::cna_picture_album_get_albums_fn=self.core.native().media.picture_album_get_albums;required_child(&self.core,&self.albums,f,PictureAlbumCollection::from_handle)}
    pub fn Pictures(&self)->Result<Arc<PictureCollection>>{let f:sys::cna_picture_album_get_pictures_fn=self.core.native().media.picture_album_get_pictures;required_child(&self.core,&self.pictures,f,PictureCollection::from_handle)}
...
impl SongCollectionExt for SongCollection {
