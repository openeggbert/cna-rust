...
use super::{native_function, MediaRuntime, MediaState, ResourceCore, Song, SongCollection, VisualizationData};
...
impl MediaPlayer {
    ...
    fn read_bool(game:&GameContext<'_>, slot:usize)->Result<bool>{let (native,handle,_)=Self::binding(game)?;let f:unsafe extern "C" fn(sys::CNA_Handle,*mut sys::CNA_Bool)->sys::CNA_Result=unsafe{native_function(slot)};let mut value=0;native.check(unsafe{f(handle,&mut value)})?;Ok(value!=0)}
    fn write_bool(game:&GameContext<'_>,slot:usize,value:bool)->Result<()>{let(native,handle,_)=Self::binding(game)?;let f:unsafe extern "C" fn(sys::CNA_Handle,sys::CNA_Bool)->sys::CNA_Result=unsafe{native_function(slot)};native.check(unsafe{f(handle,u8::from(value))})}
    fn invoke(game:&GameContext<'_>,slot:usize)->Result<()>{let(native,handle,_)=Self::binding(game)?;let f:unsafe extern "C" fn(sys::CNA_Handle)->sys::CNA_Result=unsafe{native_function(slot)};native.check(unsafe{f(handle)})}
    ...
    pub fn State(game:&GameContext<'_>)->Result<MediaState>{let(native,handle,_)=Self::binding(game)?;let f:sys::cna_media_player_get_state_fn=unsafe{native_function(native.media.media_player_get_state)};let mut value=0;native.check(unsafe{f(handle,&mut value)})?;MediaState::from_native(value)}
    pub fn PlayPosition(game:&GameContext<'_>)->Result<TimeSpan>{let(native,handle,_)=Self::binding(game)?;let f:sys::cna_media_player_get_play_position_ticks_fn=unsafe{native_function(native.media.media_player_get_play_position_ticks)};let mut value=0;native.check(unsafe{f(handle,&mut value)})?;Ok(TimeSpan::from_ticks(value))}
    pub fn Volume(game:&GameContext<'_>)->Result<f32>{let(native,handle,_)=Self::binding(game)?;let f:sys::cna_media_player_get_volume_fn=unsafe{native_function(native.media.media_player_get_volume)};let mut value=0.0;native.check(unsafe{f(handle,&mut value)})?;Ok(value)}
    pub fn SetVolume(game:&GameContext<'_>,value:f32)->Result<()>{let(native,handle,_)=Self::binding(game)?;let f:sys::cna_media_player_set_volume_fn=unsafe{native_function(native.media.media_player_set_volume)};let value=if value<0.0{0.0}else if value>1.0{1.0}else{value};native.check(unsafe{f(handle,value)})}
    ...
    pub fn Queue(game:&GameContext<'_>)->Result<Arc<MediaQueue>>{
        ...
        let f:sys::cna_media_player_get_queue_fn=unsafe{native_function(native.media.media_player_get_queue)};let mut queue_handle=0;native.check(unsafe{f(handle,&mut queue_handle)})?;let queue=MediaQueue::from_handle(Arc::clone(&native),Arc::clone(game.media_runtime()),generation,queue_handle);*cache=Some((generation,Arc::downgrade(&queue)));Ok(queue)
    }
    ...
    pub fn Play(game:&GameContext<'_>,song:&Song)->Result<()>{let(native,handle,generation)=Self::binding(game)?;if song.core.generation()!=generation{return Err(CnaError::InvalidInput("Song belongs to another Game generation"));}Self::invalidate_queue_songs_for_generation(generation);let f:sys::cna_media_player_play_song_fn=unsafe{native_function(native.media.media_player_play_song)};native.check(unsafe{f(handle,song.native_handle()?)})}
    pub fn PlayWithSongs(game:&GameContext<'_>,songs:&SongCollection)->Result<()>{let(native,handle,generation)=Self::binding(game)?;Self::invalidate_queue_songs_for_generation(generation);let f:sys::cna_media_player_play_songs_fn=unsafe{native_function(native.media.media_player_play_songs)};native.check(unsafe{f(handle,songs.native_handle()?)})}
    pub fn PlayWithSongsAndIndex(game:&GameContext<'_>,songs:&SongCollection,index:i32)->Result<()>{let(native,handle,generation)=Self::binding(game)?;Self::invalidate_queue_songs_for_generation(generation);let f:sys::cna_media_player_play_songs_from_fn=unsafe{native_function(native.media.media_player_play_songs_from)};native.check(unsafe{f(handle,songs.native_handle()?,index)})}
    ...
    pub(crate) fn play_from_event(song: &Song) -> Result<()> {
        ...
        let play: sys::cna_media_player_play_song_fn = unsafe {
            native_function(native.media.media_player_play_song)
        };
    ...
    pub(crate) fn stop_from_event() -> Result<()> {
        ...
        let stop: unsafe extern "C" fn(sys::CNA_Handle) -> sys::CNA_Result = unsafe {
            native_function(native.media.media_player_stop)
        };
...
impl MediaQueue {
    fn from_handle(native:Arc<crate::native::Native>,runtime:Arc<MediaRuntime>,generation:u64,handle:sys::CNA_Handle)->Arc<Self>{let destroy:sys::cna_media_queue_destroy_fn=unsafe{native_function(native.media.media_queue_destroy)};Arc::new(Self{core:ResourceCore::new(native,runtime,generation,handle,None,destroy,None),songs:Mutex::new(Vec::new())})}
    pub fn Count(&self)->Result<i32>{let f:sys::cna_media_queue_get_count_fn=unsafe{native_function(self.core.native().media.media_queue_get_count)};let mut value=0;self.core.native().check(unsafe{f(self.core.handle()?,&mut value)})?;Ok(value)}
    pub fn ActiveSongIndex(&self)->Result<i32>{let f:sys::cna_media_queue_get_active_song_index_fn=unsafe{native_function(self.core.native().media.media_queue_get_active_song_index)};let mut value=0;self.core.native().check(unsafe{f(self.core.handle()?,&mut value)})?;Ok(value)}
    pub fn SetActiveSongIndex(&self,value:i32)->Result<()>{let f:sys::cna_media_queue_set_active_song_index_fn=unsafe{native_function(self.core.native().media.media_queue_set_active_song_index)};self.core.native().check(unsafe{f(self.core.handle()?,value)})}
    pub fn Item(&self,index:i32)->Result<Arc<Song>>{let count=self.Count()?;if index<0||index>=count{return Err(CnaError::InvalidInput("media queue index is out of range"));}let position=index as usize;let mut cache=self.songs.lock().unwrap_or_else(std::sync::PoisonError::into_inner);if cache.len()<count as usize{cache.resize_with(count as usize,||None);}if let Some(song)=cache[position].as_ref(){return Ok(Arc::clone(song));}let f:sys::cna_media_queue_get_at_fn=unsafe{native_function(self.core.native().media.media_queue_get_at)};let mut handle=0;self.core.native().check(unsafe{f(self.core.handle()?,index,&mut handle)})?;let song=Song::from_handle(Arc::clone(self.core.native()),Arc::clone(self.core.runtime()),self.core.generation(),handle);cache[position]=Some(Arc::clone(&song));Ok(song)}
    pub fn ActiveSong(&self)->Result<Option<Arc<Song>>>{let index=self.ActiveSongIndex()?;if index<0{return Ok(None);}let f:sys::cna_media_queue_get_active_song_fn=unsafe{native_function(self.core.native().media.media_queue_get_active_song)};let mut handle=0;let mut available=0;self.core.native().check(unsafe{f(self.core.handle()?,&mut handle,&mut available)})?;if available==0{return Ok(None);}let position=index as usize;let count=self.Count()?;let mut cache=self.songs.lock().unwrap_or_else(std::sync::PoisonError::into_inner);if cache.len()<count.max(0) as usize{cache.resize_with(count.max(0) as usize,||None);}if position<cache.len(){if let Some(song)=cache[position].as_ref(){let destroy:sys::cna_song_destroy_fn=unsafe{native_function(self.core.native().media.song_destroy)};self.core.native().check(unsafe{destroy(handle)})?;return Ok(Some(Arc::clone(song)));}let song=Song::from_handle(Arc::clone(self.core.native()),Arc::clone(self.core.runtime()),self.core.generation(),handle);cache[position]=Some(Arc::clone(&song));return Ok(Some(song));}Ok(Some(Song::from_handle(Arc::clone(self.core.native()),Arc::clone(self.core.runtime()),self.core.generation(),handle)))}
