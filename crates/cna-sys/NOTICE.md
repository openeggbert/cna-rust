...
CNA Rust (`cna-sys`) is licensed under the Microsoft Public License (Ms-PL).

This workspace contains Rust bindings for
[CNA](https://github.com/openeggbert/cna), distributed separately from the
native engine and intended to communicate with it only through CNA's stable C
ABI.
...
Sharp Runtime API or ABI is exposed by either Rust crate. Portions of CNA are
derived from or based on FNA, licensed under the Ms-PL, copyright 2009-2021
Ethan Lee and the MonoGame Team. This workspace does not include FNA source.
