...
| RUST-CENSUS-002 | Bound routes with no safe call site | this repository | IN_PROGRESS: the number was wrong three times before it was right. 894, then 1,077, then 303 -- all from name matching, which cannot work here because the field holding a route's pointer is not named after the route (`cna_audio_category_pause` lives in `AudioApi::category_pause`). `tools/c-api-inventory/reachability.py` now ties each route to its field through its own `symbol!` acquisition and walks the call graph from the safe layer, with no hop limit. Measured: **138**. Each needs a safe caller or a justification in `classification.json`'s `bindingUnreachable`, and the gate fails on an unjustified one | IN_PROGRESS |
...
| RUST-CENSUS-004 | Decide the routes CNA added after the census reached zero | canonical headers | IN_PROGRESS: cnanext `7712534d3` (CABI-49) added `cna_network_session_replace_session_properties`, which arrived `UNREVIEWED` because no rule had ever seen it. A route CNA adds between milestones has to be decided before the next handoff, not carried | IN_PROGRESS |
