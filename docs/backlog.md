...
| RUST-EXT-015a | `.cnb` byte writer and reader primitives, the CNJ result, animation clips and song decode | `cnb.h` | 177 routes still unbound. `RUST-EXT-006` and `RUST-EXT-013a/b/c` bound the container, the model graph, the loader registry and the font/sound codecs; these are the layer beneath them -- the typed byte writer and reader, the `.cnj` conversion result, and the model getters and setters `013a` did not need | ACTIONABLE_LOCAL |
...
| RUST-EXT-015g | Model cameras, skins, material variants and the glTF import report | `models.h` | 34 routes reachable only through a native `CNA_ModelHandle`. This crate's `Model` is a managed Rust projection with none, exactly as `ModelMeshPart` was before `cna::extensions::engine::NativeMeshPart` was added; the same shape of answer would unblock these | BLOCKED_BY_PROJECTION |
| RUST-EXT-015c | The remaining shader-effect setters, the colour-matrix and environment-map effects | `effects.h` | 52 routes still unbound. `cna_shader_effect_set_*` is the parameter-setting half of the factory `RUST-EXT-010b` bound, so an effect acquired from the factory can be created but not fed | ACTIONABLE_LOCAL |
...
| RUST-EXT-015e | The rest of the extension surface, by header | `graphics_device.h` (24), `content_readers.h` (23), `input_gamepad.h` (23), `sensors.h` (24), `content.h` (12), and 62 more across sixteen headers | 199 routes, each in a family a DONE ticket bound the useful half of. Enumerated so they are tracked rather than invisible: the inventory reports 0 `UNMAPPED_REQUIRES_REVIEW`, which says every route has a *classification*, not that every route is bound | ACTIONABLE_LOCAL |
| RUST-EXT-015f | Value-identity routes: `_equals`, `_get_hash_code`, `_copy_string`, `_init_from_values` | across the extension headers | 48 routes. Rust implements these natively through `PartialEq`, `Hash` and `Display` on the projected values, so binding them would add a second, divergent answer to a question the Rust type already answers | OUT_OF_SCOPE |
