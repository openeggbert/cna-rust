...
| RUST-EXT-015b | Skinned models, morph targets, animation players and skinning data | `models.h` | 123 routes still unbound, and the ones the engine layer most wants: `SkinnedPbrEffect` takes a bone palette this crate cannot yet fill from a model, and `NativeMeshPart` gives the engine layer geometry the model families would supply | ACTIONABLE_LOCAL |
